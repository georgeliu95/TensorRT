...
Normalize::Normalize(const Weights* weights, int nbWeights, bool acrossSpatial, bool channelShared,
                     float eps)
...
void Normalize::configureWithFormat(const Dims* inputDims, int nbInputs, const Dims* outputDims, int nbOutputs, nvinfer1::DataType type, nvinfer1::PluginFormat format, int maxBatchSize)
{
...
const char* Normalize::getPluginType() const { return NORMALIZE_PLUGIN_NAME; }

const char* Normalize::getPluginVersion() const { return NORMALIZE_PLUGIN_VERSION; }

void Normalize::destroy() { delete this; }

IPluginV2* Normalize::clone() const
{
    IPluginV2* plugin = new Normalize(&mWeights, mNbWeights, acrossSpatial, channelShared, eps);
...
IPluginV2* NormalizePluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc)
{
    ...
    return new Normalize(&weights, mNbWeights, mAcrossSpatial, mChannelShared, mEps);
}

IPluginV2* NormalizePluginCreator::deserializePlugin(const char* name, const void* serialData, size_t serialLength)
{
    //This object will be deleted when the network is destroyed, which will
    //call Normalize::destroy()
    return new Normalize(serialData, serialLength);
}
