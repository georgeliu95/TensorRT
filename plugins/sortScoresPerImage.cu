...
ssdStatus_t sortScoresPerImage_gpu(
    cudaStream_t stream,
...
typedef ssdStatus_t (*sspiFunc)(cudaStream_t,
                                const int,
                                const int,
                                void*,
                                void*,
...
struct sspiLaunchConfig
{
    ...
    sspiLaunchConfig(DType_t t_score)
        : t_score(t_score)
    {
    }
    sspiLaunchConfig(DType_t t_score, sspiFunc function)
        : t_score(t_score)
        , function(function)
    {
    }
