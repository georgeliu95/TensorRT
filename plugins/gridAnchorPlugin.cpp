...
int GridAnchorGenerator::enqueue(int batchSize, const void* const* inputs, void** outputs, void* workspace, cudaStream_t stream)
{
    for (int id = 0; id < mNumLayers; id++)
    {
...
size_t GridAnchorGenerator::getSerializationSize() const
{
    ...
    for (int i = 0; i < mNumLayers; i++)
    {
        ...
        sum += (6 + mParam[i].numAspectRatios) * sizeof(float); // mParam[i].{minSize, maxSize, aspectRatios, variance[4]}
...
void GridAnchorGenerator::configureWithFormat(const Dims* inputs, int nbInputs, const Dims* outputDims, int nbOutputs, nvinfer1::DataType type, nvinfer1::PluginFormat format, int maxBatchSize)
{
...
const char* GridAnchorGenerator::getPluginType() const { return GRID_ANCHOR_PLUGIN_NAME; }

const char* GridAnchorGenerator::getPluginVersion() const { return GRID_ANCHOR_PLUGIN_VERSION; }

void GridAnchorGenerator::destroy() { delete this; }

IPluginV2* GridAnchorGenerator::clone() const
{
    IPluginV2* plugin = new GridAnchorGenerator(mParam.data(), mNumLayers);
...
IPluginV2* GridAnchorPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc)
{
    ...
    return new GridAnchorGenerator(boxParams.data(), numLayers);
}

IPluginV2* GridAnchorPluginCreator::deserializePlugin(const char* name, const void* serialData, size_t serialLength)
{
    //This object will be deleted when the network is destroyed, which will
    //call GridAnchor::destroy()
    return new GridAnchorGenerator(serialData, serialLength);
}
