...
ssdStatus_t sortScoresPerClass_gpu(
    cudaStream_t stream,
    const int num,
    const int num_classes,
    const int num_preds_per_class,
...
typedef ssdStatus_t (*sspcFunc)(cudaStream_t,
                                const int,
                                const int,
                                const int,
                                const int,
...
struct sspcLaunchConfig
{
    ...
    sspcLaunchConfig(DType_t t_score)
        : t_score(t_score)
    {
    }
    sspcLaunchConfig(DType_t t_score, sspcFunc function)
        : t_score(t_score)
        , function(function)
    {
    }
