...
template <typename Storage, typename Input_Data_Type, typename Output_Data_Type, int THREADS_PER_CTA,
    int THREADS_PER_PIXEL, int PIXELS_PER_THREAD_IN_REGISTERS, int PIXELS_PER_THREAD_IN_SMEM, int ELEMENTS_PER_LDG,
    int USE_ONLINE_APPROACH, int OUTER_LOOPS_, int DESIRED_OCCUPANCY>
__global__ __launch_bounds__(THREADS_PER_CTA, DESIRED_OCCUPANCY) void instance_norm_fwd(InstanceNormFwdParams params)
{
...
#pragma unroll
        ...
        int global_thread_c_input = (IS_INPUT_INT8 || IS_OUTPUT_INT8)
        ...
        int global_thread_c_output = (IS_OUTPUT_INT8)
            ? thread_in_cta_c * ELEMENTS_PER_LDG + (cta_c % 32) // handle C_ELEMENTS_PER_CTA == 16 case
                + (cta_c / 32) * 32 * params.nhw
            : thread_c;
        int stride_c_output = (IS_OUTPUT_INT8) ? 32 : params.c;
        // The base pointer to load from.
        const Input_Data_Type* gmem_src
            = &reinterpret_cast<Input_Data_Type*>(params.gmem_src)[global_thread_c_input + global_batch_offset];
...
#pragma unroll
        ...
        {
            write_to_gmem(params.gmem_saved_mean + global_stats_offset, thread_c / ELEMENTS_PER_LDG, m1);
            write_to_gmem(params.gmem_saved_var + global_stats_offset, thread_c / ELEMENTS_PER_LDG, svarinv);
        }
        ...
        {
            read_from_gmem(rmean, params.gmem_running_mean + global_stats_offset, thread_c / ELEMENTS_PER_LDG);
            read_from_gmem(rvar, params.gmem_running_var + global_stats_offset, thread_c / ELEMENTS_PER_LDG);
        }
#pragma unroll
        for (int i = 0; i < ELEMENTS_PER_LDG; ++i)
        {
...
template <typename Kernel_params>
void instance_norm_buffer_sizes(
    const InstanceNormFwdParams& params, size_t& size_sums, size_t& size_counts, size_t& size_retired_ctas)
{
    ...
    {                                                                                                                  \
        ...
        if (smem_size > 0)                                                                                             \
            CHECK_CUDA(cudaFuncSetAttribute(                                                                           \
                instance_norm_fwd<typename Kernel_params::StorageType, typename Kernel_params::Input_Data_Type,        \
...
void instance_norm_buffer_sizes_dispatch(const InstanceNormFwdContext& context, const InstanceNormFwdParams& params,
    size_t& size_sums, size_t& size_counts, size_t& size_retired_ctas, int input_data_type, int output_data_type)
{
    if (input_data_type == 2 && output_data_type == 2)
    {
        ...
        {
        case 700:
            return instance_norm_buffer_sizes<kernel_params_32_int8_sm_700>(
                params, size_sums, size_counts, size_retired_ctas);
            break;
        case 720:
            return instance_norm_buffer_sizes<kernel_params_32_int8_sm_720>(
                params, size_sums, size_counts, size_retired_ctas);
            break;
        case 750:
            return instance_norm_buffer_sizes<kernel_params_32_int8_sm_750>(
                params, size_sums, size_counts, size_retired_ctas);
            break;
        case 800:
            return instance_norm_buffer_sizes<kernel_params_32_int8_sm_800>(
                params, size_sums, size_counts, size_retired_ctas);
            break;
        case 860:
            return instance_norm_buffer_sizes<kernel_params_32_int8_sm_860>(
                params, size_sums, size_counts, size_retired_ctas);
            break;
        default:
    ...
    {
        return instance_norm_buffer_sizes<kernel_params_32_fp16_int8>(
            params, size_sums, size_counts, size_retired_ctas);
    }
...
int instance_norm_fwd_dispatch(const InstanceNormFwdContext& context, InstanceNormFwdParams& params,
    cudaStream_t stream, int input_data_type, int output_data_type)
{
