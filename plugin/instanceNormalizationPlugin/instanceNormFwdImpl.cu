 ...
 */
 ...
 __global__ __launch_bounds__(THREADS_PER_CTA, DESIRED_OCCUPANCY)
     ...
     const int32_t PIXELS_PER_CTA_IN_REGISTERS = PIXELS_PER_THREAD_IN_REGISTERS * PIXELS_PER_LDG;
     ...
     for( int32_t loop_i = 0; loop_i < OUTER_LOOPS; ++loop_i ) {
         ...
         if( USE_ONLINE_APPROACH ) {
             ...
             for( int32_t i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i ) {
                 const int32_t idx = nhw_regs + thread_in_cta_nhw + i*PIXELS_PER_LDG;
                 zero(x_storage[i]);
                 is_valid[i] = 0.f;
     ...
     if( threadIdx.x < THREADS_PER_PIXEL ) {
         const int32_t idx = blockIdx.x*THREADS_PER_PIXEL + threadIdx.x;
     ...
     if( is_valid_for_saving ) {
         writeToGmem(params.gmem_saved_mean + global_stats_offset, \
     ...
     if( params.exp_avg_factor != 1.f && is_valid_for_saving ) {
         readFromGmem(rmean, params.gmem_running_mean + global_stats_offset, \
                        thread_c/ELEMENTS_PER_LDG);
         readFromGmem(rvar, params.gmem_running_var + global_stats_offset, \
                        thread_c/ELEMENTS_PER_LDG);
     }
     #pragma unroll
     for( int32_t i = 0; i < ELEMENTS_PER_LDG; ++i ) {
 ...
 int32_t instance_norm_fwd_launch(const InstanceNormFwdContext& context, InstanceNormFwdParams& params, cudaStream_t stream)
 {
    ...
    // Synchronization is required to prevent a hang during kernel launch.
     ...
     if( int32_t(grid_dim.x) <= max_grid ) {
         ...
         grid_dim.x = max_grid;
         int32_t nhw_in_regs = params.nhw - Kernel_params::PIXELS_PER_THREAD_IN_SMEM*Kernel_params::PIXELS_PER_LDG*grid_dim.x;
         int32_t pixels_per_iteration = Kernel_params::PIXELS_PER_THREAD_IN_REGISTERS*Kernel_params::PIXELS_PER_LDG*grid_dim.x;
