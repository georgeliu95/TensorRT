...
|`int`     |`use_int8_scale_max`                     |  2, 3                             |Whether to use INT8 scale factors to optimize softmax MAX reduction. Only active when `type_id==2`. (0: disable, 1: enable), default 0.
...
This optimization was previously applied always when `type_id==2` but now is disabled by default.
