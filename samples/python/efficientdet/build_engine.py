...
class EngineBuilder:
    ...
    def create_engine(self, engine_path, precision, calib_input=None, calib_cache=None, calib_num_images=None):
        """
        ...
        if precision == "fp16":
            ...
            if not self.builder.platform_has_fast_int8:
                ...
                if not os.path.exists(calib_cache):
                    calib_shape = [8] + list(inputs[0].shape[1:])  # Always use a batch size of 8 for calibration
...
if __name__ == "__main__":
    ...
    if args.precision == "int8" and not any([args.calib_input, args.calib_cache]):
        parser.print_help()
        log.error("When building in int8 precision, --calib_input or --calib_cache is required")
        sys.exit(1)
    main(args)
