...
TensorRT engines for [TensorFlow 2 Object Detection's](https://github.com/tensorflow/models/blob/master/research/object_detection/g3doc/tf2_detection_zoo.md) Single-Shot Detector, Faster R-CNN and Mask R-CNN models.
...
If you would like to use Docker, we have tested and suggesting `docker pull nvcr.io/nvidia/tensorflow:21.07-tf2-py3` as a base image. 
...
You will also need the `onnx_graphsurgeon` python module. If not already installed by TensorRT, you can install it manually by running:
...
Finally, you need to install [TensorFlow 2 Object Detection API](https://github.com/tensorflow/models/blob/master/research/object_detection/g3doc/tf2.md) to use some helper utilities from it.


## Model Conversion

The workflow to convert an TFOD model is basically TensorFlow → ONNX → TensorRT, and so parts of this process require TensorFlow to be installed. If you are performing this conversion to run inference on the edge, such as for NVIDIA Jetson devices, it might be easier to do the ONNX conversion on a PC first.
...
└── saved_model
    └── saved_model.pb
```
In order to proceed, you need to re-export the saved model. If you won't re-export the model with with **float_image_tensor** conversion process will **fail**.
...
When you will try to re-export the Mask R-CNN model, you may be greeted with an error:
...
This error comes from a bug inside of the `/mask_rcnn_inception_resnet_v2_1024x1024_coco17_gpu-8/pipeline.config` file, in order to fix it, all you have to do is find a line with `input_path: "PATH_TO_BE_CONFIGURED"s`, currently it is line **153** and delete `s` on the end of this line, save the file and run re-export one more time.
...
The outputs of the graph are the same as the outputs of the [EfficientNMS_TRT](https://github.com/NVIDIA/TensorRT/tree/master/plugin/efficientNMSPlugin) plugin. In case of **Mask R-CNN** graph will have one more additional output from a segmentation head, name of the last node is `Reshape_Final_Masks`, shape is `[batch_size, max_proposals, mask_height, mask_width]`, dtype is float32. 
...
Where the `--preprocessor` corresponds with image preprocessor set in your `pipeline.config` file as described in the inference section above.
