...
void printUsage()
{
    ...
    printf("  --padMultiple=N                      Specify multiple to pad out matrix dimensions to test performance\n");
}
    ...
    nmtSample::Alignment::ptr alignment)
{
    ...
    nvinfer1::INetworkConfig* encoderConfig = encoderBuilder->createNetworkConfig();
    ...
    nmtSample::Likelihood::ptr likelihood)
{
    ...
    nvinfer1::INetworkConfig* generatorConfig = generatorBuilder->createNetworkConfig();
...
nvinfer1::ICudaEngine* getGeneratorShuffleEngine(const std::vector<nvinfer1::Dims>& decoderStateSizes, int attentionSize)
{
    ...
    nvinfer1::INetworkConfig* shuffleConfig = shuffleBuilder->createNetworkConfig();
...
int main(int argc, char** argv)
{
    ...
    dataWriter->finalize();
    float score = gDataWriterStr == "bleu" ? static_cast<nmtSample::BLEUScoreWriter*>(dataWriter.get())->getScore() : -1.0f;

    if (gDataWriterStr == "benchmark")
    {
        gLogInfo << "Average latency (without data read) = " << totalLatency / static_cast<float>(batchCount) << " ms" << std::endl;
    }
