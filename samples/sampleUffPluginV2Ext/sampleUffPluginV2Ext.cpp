...
public:
    ...
    bool build()
    {
        ...
        SampleUniquePtr<INetworkConfig> networkConfig{builder->createNetworkConfig()};
        networkConfig->setMaxWorkspaceSize(1_GB);
        if (gArgs.runInFp16)
        {
...
public:
    ...
    UffPoolPluginV2(const void* data, size_t length)
    {
        ...
        {
            mInHostScale.resize(read<int>(d));
            std::for_each(mInHostScale.begin(), mInHostScale.end(), [&](float& n) { n = read<float>(d); });
            mOutHostScale.resize(read<int>(d));
            std::for_each(mOutHostScale.begin(), mOutHostScale.end(), [&](float& n) { n = read<float>(d); });
        }
        assert(d == a + length);
    }
...
public:
    ...
    size_t getSerializationSize() const override
    {
        ...
        if (mDataType == DataType::kINT8)
        {
            serializationSize += sizeof(int);
            serializationSize += sizeof(float) * mInHostScale.size();
            serializationSize += sizeof(int);
            serializationSize += sizeof(float) * mOutHostScale.size();
        }
    ...
    void serialize(void* buffer) const override
    {
        ...
        if (mDataType == DataType::kINT8)
        {
            write(d, static_cast<int>(mInHostScale.size()));
            std::for_each(mInHostScale.begin(), mInHostScale.end(), [&](float n) { write(d, n); });
            write(d, static_cast<int>(mOutHostScale.size()));
            std::for_each(mOutHostScale.begin(), mOutHostScale.end(), [&](float n) { write(d, n); });
        }
        assert(d == a + getSerializationSize());
    }

    void configurePlugin(const PluginConfig* in, int nbInput, const PluginConfig* out, int nbOutput) override
    {
        assert(in && nbInput == 1);
        ...
        if (in[0].hostScale)
        {
            assert(in[0].type == DataType::kINT8);
            mInHostScale.clear();
            std::copy(in[0].hostScale, in[0].hostScale + getC(in[0].dims), std::back_inserter(mInHostScale));
        }
        if (out[0].hostScale)
        {
            assert(out[0].type == DataType::kINT8);
            mOutHostScale.clear();
            std::copy(out[0].hostScale, out[0].hostScale + getC(out[0].dims), std::back_inserter(mOutHostScale));
        }
    }

    bool supportsFormat(const PluginConfig* in, int nbInput, const PluginConfig* out, int nbOutput) const override
    {
        assert(nbInput <= 1 && nbOutput <= 1);
        if (in && (in[0].format != TensorFormat::kLINEAR || in[0].type == DataType::kINT32))
        {
            return false;
        }
        if (out && (out[0].format != TensorFormat::kLINEAR || out[0].type == DataType::kINT32))
        {
            return false;
        }
    ...
    DataType getOutputDataType(int index, const PluginConfig* config, int nbInputs) const override
    {
        // Input and output data types are the same in this sample
        assert(nbInputs == 1 && config);
        assert(index == 0);
        return config[0].type;
    }

    TensorFormat getOutputTensorFormat(int index, const PluginConfig* config, int nbInputs) const override
    {
        assert(nbInputs == 1 && config);
        assert(index == 0);
...
private:
    ...
    void copyDeviceInputToFP32(const void* src, void*& dst)
    {
        ...
        // int8 scale
        assert(mInputDims.d[0] == int(mInHostScale.size()));
        int hw = mInputDims.d[1] * mInputDims.d[2];
        for (int j = 0; j < mInputDims.d[0]; ++j)
        {
            std::transform(inputFP32.get() + hw * j, inputFP32.get() + hw * (j + 1), inputFP32.get(),
                [&](float in) -> float { return in * mInHostScale[j]; });
        }
    ...
    void copyDeviceToInt8Output(const void* src, void* dst)
    {
        ...
        CHECK(cudaMemcpy(outTmp.get(), src, outCount * elementSize(mDataType), cudaMemcpyDeviceToHost));
        std::unique_ptr<char> outInt8{new char[outCount * elementSize(DataType::kINT8)]};
        // int8 + scale
        assert(mOutputDims.d[0] == int(mOutHostScale.size()));
        int hw = mOutputDims.d[1] * mOutputDims.d[2];
        for (int j = 0; j < mInputDims.d[0]; ++j)
        {
            std::transform(outTmp.get() + hw * j, outTmp.get() + hw * (j + 1), outTmp.get(),
                [&](float in) -> float { return in / mOutHostScale[j]; });
        }
...
private:
    ...
    Dims mInputDims;
    Dims mOutputDims;
    std::vector<float> mInHostScale;
    std::vector<float> mOutHostScale;
