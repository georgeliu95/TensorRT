...
1. The sample gets compiled when building the TensorRT OSS following the [instructions](https://github.com/NVIDIA/TensorRT). The binary named `sample_onnx_mnist_coord_conv_ac` will be created in the `<TensorRT root directory>/build/out/` directory, where `<TensorRT root directory>` is where you installed TensorRT.
