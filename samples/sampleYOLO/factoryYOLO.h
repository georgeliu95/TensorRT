...
// LReLU parameter
static const float kNEG_SLOPE = 0.1f;

//Num of lReLU layers in the YOLO network
#define NUM_RELU_IN_YOLO 25

bool matchString(const char* pattern, const char* name)
{
...
class YOLOPluginFactory : public nvcaffeparser1::IPluginFactoryV2
{
public:
    YOLOPluginFactory()
    {
        for (unsigned i = 0; i < NUM_RELU_IN_YOLO; i++)
        {
    ...
    virtual nvinfer1::IPluginV2* createPlugin(const char* layerName, const nvinfer1::Weights* weights, int nbWeights, const char* libNamespace) override
    {
        ...
        const int i = mReLULayerNum1;
        assert(i >= 0 && i < NUM_RELU_IN_YOLO);
        assert(mPluginLRelu[i] == nullptr);
        assert(nbWeights == 0 && weights == nullptr);
        mPluginLRelu[i] = std::unique_ptr<IPluginV2, void (*)(IPluginV2*)>(createLReLUPlugin(kNEG_SLOPE), pluginDeleter);
        mReLULayerNum1++;
