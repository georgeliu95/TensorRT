...
**TensorRT GA build**
* [TensorRT](https://developer.nvidia.com/nvidia-tensorrt-download) v8.2.1.1
...
2. #### (Optional - if not using TensorRT container) Specify the TensorRT GA release build
    ...
    **Example: Ubuntu 18.04 on x86-64 with cuda-11.4**
    ...
    ```bash
    cd ~/Downloads
    tar -xvzf TensorRT-8.2.1.1.Linux.x86_64-gnu.cuda-11.4.cudnn8.2.tar.gz
    export TRT_LIBPATH=`pwd`/TensorRT-8.2.1.1
    ```
    ...
    **Example: Windows on x86-64 with cuda-11.4**
    ...
    ```powershell
    cd ~\Downloads
    Expand-Archive .\TensorRT-8.2.1.1.Windows10.x86_64.cuda-11.4.cudnn8.2.zip
    $Env:TRT_LIBPATH = '$(Get-Location)\TensorRT-8.2.1.1'
