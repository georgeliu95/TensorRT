...
**TensorRT GA build**
* [TensorRT](https://developer.nvidia.com/nvidia-tensorrt-download) v8.4.1.4
...
2. #### (Optional - if not using TensorRT container) Specify the TensorRT GA release build
    ...
    **Example: Ubuntu 20.04 on x86-64 with cuda-11.6.2**
    ...
    ```bash
    cd ~/Downloads
    tar -xvzf TensorRT-8.4.1.4.Linux.x86_64-gnu.cuda-11.6.cudnn8.4.tar.gz
    export TRT_LIBPATH=`pwd`/TensorRT-8.4.1.4
    ```
    ...
    **Example: Windows on x86-64 with cuda-11.4**
    ...
    ```powershell
    cd ~\Downloads
    Expand-Archive .\TensorRT-8.4.1.4.Windows10.x86_64.cuda-11.6.cudnn8.4.zip
    $Env:TRT_LIBPATH = '$(Get-Location)\TensorRT-8.4.1.4'
