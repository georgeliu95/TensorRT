...
def emb_layernorm(builder, network, config, weights_dict, builder_config, sequence_length, batch_sizes):
    ...
    use_full_mask = 0
    # use full_mask for XMMA kernels when SM version >= 75(sequence 64/96/128/384 for fp16 precision, sequence 128/384 for int8 precision)
    cc = pycuda.autoinit.device.compute_capability()
    if not config.is_calib_mode and cc[0] * 10 + cc[1] >= 75:
        if config.use_fp16 and (sequence_length == 64 or sequence_length == 96 or sequence_length == 128 or sequence_length == 384):
            use_full_mask = 1
        if config.use_int8 and (sequence_length == 128 or sequence_length == 384):
            use_full_mask = 1
