...
class BartMetadata(_BartMetadata, MetadataArgparseInteropMixin):
    ...
    def from_args(args: argparse.Namespace):
        return NetworkMetadata(
            ...
            precision=Precision(fp16=False, noTF32=False),
    ...
    def add_inference_args(parser: argparse.ArgumentParser) -> None:
        ...
        inference_group.add_argument(
            "--noTF32", action="store_true", help="Enables fp32 TensorRT tactics."
        )

    @staticmethod
    def from_inference_args(args: argparse.Namespace):
        base_metadata = BartMetadata.from_args(args)
        return base_metadata._replace(precision=Precision(fp16=args.fp16, noTF32=args.noTF32))
...
class BartModelTRTConfig(NNConfig):
    ...
    def __init__(self):
        precisions = ['noTF32', 'tf32', 'fp16']
        kv_caches = [False, True]

        variants = []
        for variant, precision, kv_cache in product(
            BartModelTRTConfig.TARGET_MODELS, precisions, kv_caches
        ):
            variants.append(
                NetworkMetadata(
                    variant=variant,
                    precision=Precision(fp16= precision=='fp16', noTF32= precision=='noTF32'),
