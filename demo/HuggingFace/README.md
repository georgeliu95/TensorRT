...
Frameworks (PyTorch) by default run TF32 on Ampere devices and FP32 on pre-Ampere devices. Therefore, in TensorRT run, TF32 is set as the default precision. To experiment with different precisions, use `--fp16` for FP16 and `--noTF32` for FP32:

```python
python3 run.py run BART trt --variant facebook/bart-base --working-dir temp [--fp16 | --noTF32]
```

## How to run with K-V cache

For BART, use `--enable-kv-cache` option to get the same effect of HuggingFace's `use_cache` option.
