...
```bash
python3 demo_img2img.py "A fantasy landscape, trending on artstation" --hf-token=$HF_TOKEN
```
...
The refinement model can be enabled using `--enable-refiner`

### Generate an image guided by a single text prompt with style from LoRA weights

```bash
python3 demo_txt2img.py "A pokemon with green eyes and red legs." --hf-token=$HF_TOKEN --version 1.5 -v --lora-weights="sayakpaul/sd-model-finetuned-lora-t4"  --lora-scale=1.0
```
