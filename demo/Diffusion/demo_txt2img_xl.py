...
if __name__ == "__main__":
    ...
    def init_pipeline(pipeline_class, refiner, onnx_dir, engine_dir):
        ...
        demo = pipeline_class(
            ...
            lora_weights=args.lora_weights if not refiner else None)
    ...
    def run_sd_xl_inference(warmup=False, verbose=False):
        return_type = "latents" if args.enable_refiner else "image"
        images, time_base = demo_base.infer(prompt, negative_prompt, image_height, image_width, warmup=warmup, verbose=verbose, seed=args.seed, return_type=return_type)
        time_refiner = 0
        if args.enable_refiner:
