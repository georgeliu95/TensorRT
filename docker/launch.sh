...
if [ "$arg_help" -eq "1" ]; then
    ...
    echo " --gpu     <number>    : Number of GPUs visible in container. Set 'none' to disable, and 'all' to make all visible."
...
extra_args=""
if [ "$arg_gpu" != "none" ]; then
    extra_args="$extra_args --gpu $arg_gpu"
fi
