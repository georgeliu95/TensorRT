...
class Model(object):
    def __init__(self, path: str, inputs: List[Tensor], outputs: List[Tensor], nodes: List[Node]):
        self.path = path
        self.inputs = inputs
...
def identity_model():
    ...
    return Model(os.path.join(TEST_ROOT, "models", "identity.onnx"), inputs=[x], outputs=[y], nodes=[node])


def lstm_model():
    path = os.path.join(TEST_ROOT, "models", "lstm.onnx")
    onnx_graph = onnx.load(path).graph
    ...
    return Model(path, inputs=[X, W, R, B, initial_c], outputs=[Y, Y_h, Y_c], nodes=[node])
...
def scan_model():
    ...
    return Model(path, inputs=inputs, outputs=outputs, nodes=[scan_node])
