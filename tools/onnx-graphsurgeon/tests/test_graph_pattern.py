...
class TestGraphPatternMatching:
    ...
    def test_callback_check_unmatch(self):
        ...
        path = os.path.join(TEST_ROOT, "models", "test_toyPlugin_callback_check_unmatch_case.onnx")
        graph = import_onnx(onnx.load(path))
    ...
    def test_intermediate_output_unmatch(self):
        ...
        path = os.path.join(TEST_ROOT, "models", "test_toyPlugin_intermediate_output_unmatch_case.onnx")
        graph = import_onnx(onnx.load(path))
    ...
    def test_intermediate_output_to_other_node_unmatch(self):
        ...
        path = os.path.join(TEST_ROOT, "models", "test_toyPlugin_intermediate_output_to_other_node_unmatch_case.onnx")
        graph = import_onnx(onnx.load(path))
...
class TestOutputNodes:
    ...
    def get_plugin_pattern_with_multiple_output_node(self):
        ...
        in_1 = pattern.variable()
        a_out = pattern.add("Anode", "A", inputs=[in_0])
        b_out_0, b_out_1= pattern.add("Bnode", "B", inputs=[in_1], num_output_tensors=2)
    ...
    def test_outbound_node_with_consumer_match(self):
        ...
        # I/O mapping
        assert inputs[0].name == "i1" and inputs[1].name == "i1"
        assert outputs[0].name == "o1" and outputs[1].name == "o2" and outputs[2].name == "i3"
...
class TestConstantCases:
    ...
    def test_constant_initializer_match(self):
        path = os.path.join(TEST_ROOT, "models", "test_toyPlugin_constant_initializer_match_case.onnx")
        graph = import_onnx(onnx.load(path))
    ...
    def test_constant_node_match(self):
        path = os.path.join(TEST_ROOT, "models", "test_toyPlugin_constant_node_match_case.onnx")
        graph = import_onnx(onnx.load(path))
